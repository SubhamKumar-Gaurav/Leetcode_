        ...
        if num_digits==1 : 
            for single_digit in range(1,10) : 
                if single_digit%divisor==0 : 
                    valid_digit_frequencies.add(freq_to_tuple(Counter(str(single_digit)))) 
        else : 
            ...
            if num_digits%2==0 : 
                for left_half in left_half_combinations : 
                    ...
                    if palindrome_value%divisor==0 : 
                        valid_digit_frequencies.add(freq_to_tuple(Counter(palindrome_str))) 
            else : 
                for left_half in left_half_combinations : 
                    if left_half[0]=="0" : 
                        continue 
